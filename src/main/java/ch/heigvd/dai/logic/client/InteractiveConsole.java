...
public class InteractiveConsole {
   ...
   */
  private static final Map<PlayerState, IInputParser> STATE_INPUT_PARSERS = Map.of(
    PlayerState.WAIT_FOR_USERNAME, new UsernameInputParser(),
      PlayerState.WAIT_IN_LOBBY, new LobbyInputParser(),
      PlayerState.WAIT_FOR_GUESS, new GuessInputParser(),
      PlayerState.SECOND_GUESS_PHASE, new SecondPhaseInputParser(),
      PlayerState.WAIT_FOR_VOWEL, new VowelInputParser(),
      PlayerState.WAIT_FOR_FILL, new FillInputParser(),
      PlayerState.SEND_LETTERS, new LettersInputParser(),
      PlayerState.WAIT_FOR_LAST_TURN, new FillInputParser()
  );

  /**
   * Parsers for a server response when it is of a specific command type
   */
  private static final Map<GameCommandType, IResponseParser> COMMAND_RESPONSE_PARSERS = Map.of(
   ...
   */
  private static final List<PlayerStatePrompt> STATE_PROMPTS = Arrays.asList(
    new PlayerStatePrompt(PlayerState.SECOND_GUESS_PHASE, new String[]
        {
            "Skip turn",
            "Buy a vowel",
            "Complete the puzzle"
        }),
    new PlayerStatePrompt(PlayerState.WAIT_FOR_USERNAME, new String[]
        {
            "Welcome! Please enter your username:"
        }),
      new PlayerStatePrompt(PlayerState.WAIT_FOR_FILL, new String[]
          {
              "Here's your chance to complete the puzzle. What is it?"
          }),
      new PlayerStatePrompt(PlayerState.WAIT_FOR_GUESS, new String[]
          {
              "Type a consonant to see if it is in the puzzle:"
          }),
      new PlayerStatePrompt(PlayerState.WAIT_FOR_VOWEL, new String[]
          {
              "Type the vowel to buy:"
          }),
      new PlayerStatePrompt(PlayerState.WAIT_FOR_LAST_TURN, new String[]
          {
              "Type the full puzzle:"
          }),
      new PlayerStatePrompt(PlayerState.SEND_LETTERS, new String[]
          {
              "Type " + LettersCommand.NumberOfLetters + " letters to complete the last puzzle:"
          }),
      new PlayerStatePrompt(PlayerState.WAIT_IN_LOBBY, new String[]
          {
  ...
  /**
   * Creates a new interactive console instance to make it easier for players to take part
   * in the game
  ...
  /**
   * Parses the provided user input and returns the corresponding game command, if any needs to
   * be sent
